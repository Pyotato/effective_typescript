...
|  35  | [데이터가 아닌, API와 명체를 보고 타입 만들기](https://github.com/Pyotato/effective_typescript/blob/item35/README.md) | 💡 코드의 구석 구석까지 타입 안전성을 얻기 위해 API또는 데이터 형식에 대한 타입 생성을 고려해야 합니다. <br/>💡 데이터에 드러나지 않는 예외적인 경우들이 문제가 돌 수 있기 때문에 데이터보다는 명세로부터 코드를 생성하는 것이 좋습니다.| 
...
|  56  |     [정보를 감추는 목적으로 private 사용하지 않기](https://github.com/Pyotato/effective_typescript/blob/item56/README.md)     | 💡 public, protected, private 접근 제어자는 타입 시스템에서만 강제될 뿐입니다. 런타임에는 소용이 없으며 단언문을 통해 우회할 수 있습니다. 접근 제어자로 데이터를 감추려고 해서는 안됩니다.<br/>💡 확실히 데이터를 갘추고 싶다면 클로저를 사용해야 합니다. |
|  57  |     [정보를 감추는 목적으로 private 사용하지 않기](https://github.com/Pyotato/effective_typescript/blob/item57/README.md)     | 💡 원본 코드가 아닌 변환된 자바스크립트 코드를 디버깅하지 맙시다. 소스맵을 사용해서 런타임에 타입스크립트 코드를 디버깅합기다. <br/>💡 소스맵이 최종적으로 변환된 코드에 완전히 매핑되었는지 확인합시다. <br/>💡 소스맵에 원본 코드가 그대로 포함되도록 설정되어 있을 수도 있습니다. 공개되지 않도록 설정을 확인합시다. |
...
|  59  |     [타입스크립트 도입 전에 @ts-check의 JSDoc으로 시험해 보기](https://github.com/Pyotato/effective_typescript/blob/item59/README.md)     | 💡 파일 상단에 `// @ts-check`를 추가하면 자바스크립트에서도 타입 체크를 수행할 수 있습니다.<br/> 💡 전역 선언과 서드파티 라이브러리의 타입 선언을 추가하는 방법을 익힙시다.<br/> 💡 JSDoc 주석을 잘 활용하면 자바스크립트 상태에서도 타입 단언과 타입 추론을 할 수 있습니다. <br/> 💡 JSDoc 주석은 중간단계이기 때문에 너무 공들일 필요는 없습니다. 최종 목표는 <i>.ts</i>로 된 타입스크립트 코드임을 명심합시다. |
|  60  |     [allowJs로 타입스크립트와 자바스크립트 같이 사용하기](https://github.com/Pyotato/effective_typescript/blob/item60/README.md)     | 💡 점진적 마이그레이션을 위해 자바스크립트와 타입스크립트를 동시에 사용할 수 있게 `allowJS` 컴파일 옵션을 사용합시다.<br/> 💡 대규모 마이그레이션 작업을 시작하기 전에, 테스트의 빌드 체인에 타입스크립트를 적용해야 합니다.|
|  61  |     [의존성 관계에 따라 모듈로 전환하기](https://github.com/Pyotato/effective_typescript/blob/item61/README.md)     | 💡 마이그레이션의 첫 단계는, 서드파티 모듈과 외부 API호출에 대한 `@types`를 추가하는 것입니다.<br/> 💡 의존성 관계도의 아래에서부터 위로 올라가며 마이그레이션을 하면 됩니다. 첫 번째 모듈은 보통 유틸리티 모듈입니다. 의존성 관계도를 시각화하여 진행 과정을 추적하는 것이 좋습니다.<br/> 💡 이상한 설계를 발견하더라도 리팩터링을 하면 안 됩니다. 마이그레이션 작업은 타입스크립트 전환에 집중해야 하며, 나중의 리팩터링을 위해 목록을 만들어 두는 것이 좋습니다.<br/> 💡 타입스크립트로 전환하며 발견하게 되는 일반적인 오류들을 놓치지 않아야 합니다. 타입 정보를 유지하기 위해 필요에 따라 JSDoc 주석을 활용해야 할 수도 있습니다.|
